...
// Nathan thinks Joseph added this
void Crazyflie::sendSetpoint(
  uint32_t packed)
{
  crtpCompactSetpointRequest request(packed);
  sendPacket((const uint8_t*)&request, sizeof(request));
}

// Added sendSetPoint with packet ID
void Crazyflie::sendSetpointID(
  uint32_t packed, uint16_t packet_ID)
{
  crtpCompactSetpointRequestwID request(packed, packet_ID);   // Nathan need to change this line
  sendPacket((const uint8_t*)&request, sizeof(request));
}
...
void Crazyflie::requestLogToc(bool forceNoCache)
{
  ...
  // ROS_INFO("Recieving LogTocs");

  // NOL changed to try and always force no Cache
  if (!infile.good()){ //} (forceNoCache || !infile.good()) {
