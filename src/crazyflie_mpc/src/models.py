...
def simulate_learned(model, actions, generalNN = True, x0=[]):
    ...
    X = [x0]
    elapsed = 0
    #print "NORMAL simulate_learned->actions:shape: ", actions.shape
    for a in actions:
        if generalNN:
     ...
     #   print "ORIGINAL xnext shape: ", xnext.shape
        X.append(xnext)
    #print "NN for ", len(actions), " actions: ", (elapsed / len(actions)), " per action"
    #print "ORIGINAL X shape = ", len(X), X[0].shape
    return np.array(X)

def simulate_learned_batch(model, actions, generalNN = True, x0=[]):
    # returns a array of the states predicted by the learned dynamics model given states and the inputs
    if (x0 == []):
        x0 = np.zeros(model.x_dim,1)
    ...
    if generalNN:
        st = utils_data.millis()
     #   print "simulate_learned->actions:shape: ", actions.shape
        xnext = predict_nn_batch(model, x0, actions[:,1,:], model.state_idx_l)
        et = utils_data.millis()
        elapsed += (et - st)
    else:
        xnext = X[-1].flatten() + model.predict(X[-1], a)
