...
#include "crazyflie_driver/MotorControlwID.h"   // changed from MotorControl wot get catkin_make to run
...
private:
   ...
   */
  void publishPackets() {
    std::vector<Crazyradio::Ack> packets = m_cf.retrieveGenericPackets(); // retrievegenericLogTopic_state_data_VariablesPackets();
    if (!packets.empty())
    {
...
void cmdPositionSetpoint(
  ...
  // New version is for the motor control packet with packet IDs
  void cmdVelChanged(
    const crazyflie_driver::MotorControlwID::ConstPtr& msg)
  {
    if (!m_isEmergency) {

      uint16_t m1 = std::min<uint16_t>(std::max<float>(msg->m1, 0.f), 65535);  // NOL changed to 65535
      uint16_t m2 = std::min<uint16_t>(std::max<float>(msg->m2, 0.f), 65535);
      uint16_t m3 = std::min<uint16_t>(std::max<float>(msg->m3, 0.f), 65535);
      uint16_t m4 = std::min<uint16_t>(std::max<float>(msg->m4, 0.f), 65535);

      uint32_t packed = ((m1 & 0xFF00) >> 8) + ((m2 & 0xFF00)) + ((m3 & 0xFF00) << 8) + ((m4 & 0xFF00) << 16);

      // Added packet_ID
      uint16_t packet_ID = msg->ID;

      ROS_INFO("packed: %X ", packed);
      ROS_INFO("packetID: %d ", packet_ID);

      m_cf.sendSetpointID(packed, packet_ID);
      m_sentSetpoint = true;
    }
  }
  ...
  void run()
  {
    ...
    // Some commented out by NOL
    m_subscribeCmdVel = n.subscribe(m_tf_prefix + "/cmd_vel", 2, &CrazyflieROS::cmdVelChanged, this);   // changed the integer argument to 2 for 2 variables?
    ...
    while(!m_isEmergency) {
      ...
      if (m_enableLogging && !m_sentSetpoint && !m_sentExternalPosition) { // CHANGED NOL
